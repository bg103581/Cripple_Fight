...
public class BasicMoveP1 : MonoBehaviour {
    ...
    void Update() {
        ...
        if (Input.GetKey("d") && ryuAnimator.GetBool("isCrouching") == false && grounded) {
        ...
        } else if (Input.GetKey("q") && ryuAnimator.GetBool("isCrouching") == false && grounded) {
    ...
    /*private void FixedUpdate() {
        if (startDelay) {
            ...
            if (timePassed <= delay) {
                if (rightPress >= 2) {
                    rbRyu.velocity = new Vector2(speed * 2, rbRyu.velocity.y);
                    rightPress = 0;
                }
                else if (leftPress >= 2) {
                    rbRyu.velocity = new Vector2(-(speed * 2), rbRyu.velocity.y);
